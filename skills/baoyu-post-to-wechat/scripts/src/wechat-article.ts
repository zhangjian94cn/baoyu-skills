...
async function copyImageToClipboard(imagePath: string): Promise<void> {
  ...
  if (process.platform === 'darwin') {
    // macOS: use osascript
    result = spawnSync('osascript', ['-e', `set the clipboard to (read (POSIX file "${absPath}") as TIFF picture)`]);
  } else if (process.platform === 'win32') {
    // Windows: use PowerShell
    const ps = `Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Clipboard]::SetImage([System.Drawing.Image]::FromFile('${absPath.replace(/'/g, "''")}'))`;
    result = spawnSync('powershell', ['-NoProfile', '-Command', ps]);
  } else {
    // Linux: use xclip
    result = spawnSync('xclip', ['-selection', 'clipboard', '-t', 'image/png', '-i', absPath]);
  }
...
async function sendCopy(cdp?: CdpConnection, sessionId?: string): Promise<void> {
  if (process.platform === 'darwin') {
    ...
    // Windows: document.execCommand('copy') 比 CDP 键盘事件更可靠
...
async function sendPaste(cdp?: CdpConnection, sessionId?: string): Promise<void> {
  if (process.platform === 'darwin') {
    ...
    // Windows: 使用 CDP 键盘事件（用于非 HTML 的简单粘贴）
...
// 缓存从 HTML tab 提取的内容，用于跨 tab 传递
let _extractedHtml: string | null = null;
...
async function copyHtmlFromBrowser(cdp: CdpConnection, htmlFilePath: string, contentImages: ImageInfo[] = []): Promise<void> {
  ...
  // 直接提取 HTML 内容（不依赖系统剪贴板）
  console.log('[wechat] Extracting #output HTML content...');
  ...
  _extractedHtml = htmlResult.result.value || null;
  console.log(`[wechat] Extracted HTML: ${_extractedHtml ? _extractedHtml.length + ' chars' : 'empty'}`);

  // macOS / Linux 仍使用剪贴板方式作为备选
...
async function pasteFromClipboardInEditor(session: ChromeSession): Promise<void> {
  if (_extractedHtml && process.platform === 'win32') {
    // Windows: 通过合成 paste 事件直接注入 HTML（绕过系统剪贴板）
    console.log('[wechat] Injecting HTML via synthetic paste event...');
    const escapedHtml = JSON.stringify(_extractedHtml);
...
async function parseMarkdownWithPlaceholders(markdownPath: string, theme?: string): Promise<{ title: string; author: string; summary: string; htmlPath: string; contentImages: ImageInfo[] }> {
  ...
  const mdToHtmlScript = path.resolve(__dirname, '../../baoyu-markdown-to-html/scripts/main.ts');
  const scriptArgs = [mdToHtmlScript, markdownPath];
  if (theme) scriptArgs.push('--theme', theme);
  ...
  const result = spawnSync(cmd, [...args], { stdio: ['inherit', 'pipe', 'pipe'], shell });
  if (result.status !== 0) {
