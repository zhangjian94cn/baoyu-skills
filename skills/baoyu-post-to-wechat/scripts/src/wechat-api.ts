...
import { getScriptDir, getProjectRoot, getEnvPaths, loadEnvFile } from "./config.ts";

// Proxy support: reads from HTTPS_PROXY, ALL_PROXY, or .env file
function getProxyUrl(): string | undefined {
  return process.env.HTTPS_PROXY || process.env.ALL_PROXY || process.env.https_proxy || process.env.all_proxy;
}

const proxyUrl = getProxyUrl();
if (proxyUrl) {
  console.error(`[wechat-api] Using proxy: ${proxyUrl}`);
}

// Create proxy agent
function createProxyAgent(): SocksProxyAgent | HttpsProxyAgent<string> | undefined {
...
// Wrapper for fetch with proxy support using node-fetch
async function proxyFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  const fetchInit: NodeFetchRequestInit = { ...(init as any) };
  if (proxyAgent) {
    fetchInit.agent = proxyAgent;
  }
  const response = await nodeFetch(url.toString(), fetchInit);
...
function loadConfig(): WechatConfig {
  ...
  for (const configPath of getEnvPaths(projectRoot)) {
    ...
    if (!appId && env.WECHAT_APP_ID) appId = env.WECHAT_APP_ID;
    if (!appSecret && env.WECHAT_APP_SECRET) appSecret = env.WECHAT_APP_SECRET;
  }

  if (!appId || !appSecret) {
    throw new Error(
      "Missing WECHAT_APP_ID or WECHAT_APP_SECRET.\n" +
      "Set via environment variables or in .env file (project root, .baoyu-skills/.env, or ~/.baoyu-skills/.env)."
    );
  }
...
async function uploadImage(
  ...
  if (imagePath.startsWith("http://") || imagePath.startsWith("https://")) {
    const response = await proxyFetch(imagePath);
    if (!response.ok) {
...
function renderMarkdownToHtml(markdownPath: string, theme: string = "default"): string {
  ...
  // 使用 sibling skill: baoyu-markdown-to-html 的渲染引擎
  const renderScript = path.resolve(__dirname, "../../../baoyu-markdown-to-html/scripts/md/render.ts");
  const baseDir = path.dirname(markdownPath);

  console.error(`[wechat-api] Rendering markdown with theme: ${theme}`);
  const isWin = os.platform() === "win32";
  const [cmd, args, shell] = isWin
    ? ["bun", [renderScript, markdownPath, "--theme", theme], false] as const
    : ["npx", ["-y", "bun", renderScript, markdownPath, "--theme", theme], true] as const;
